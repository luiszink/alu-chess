...
object TournamentBot:
  ...
  def run(
    ...
    def handleEvent(event: TournamentEvent): IO[Unit] =
      event match
    ...
    def streamLoop: IO[Unit] =
      client
        ...
        .handleErrorWith { error =>
          log(s"Stream error: ${error.getMessage}; reconnecting in 3 seconds...") *>
            statusRef.update {
              case BotStatus.Error(_) => BotStatus.InTournament(tournamentId, 0, 0)
              case state              => state
            } *>
            IO.sleep(3.seconds) *>
            streamLoop
